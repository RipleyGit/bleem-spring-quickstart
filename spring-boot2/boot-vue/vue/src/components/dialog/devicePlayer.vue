      ...
      <div id="shared" style="text-align: right; margin-top: 1rem;">
        <el-tabs v-model="tabActiveName" @tab-click="tabHandleClick">
          ...
          <el-tab-pane style="margin-left: 50%;" label="语音对讲" name="talkback">
            <div>
              对讲模式：
              <el-select v-model="broadCastModel" placeholder="请选择" :disabled="talkStatus">
                <el-option v-for="item in broadCastOption" :key="item.key" :label="item.key" :value="item.value">
                </el-option>
              </el-select>
            </div>
          ...
          <el-tab-pane style="margin-left: 50%;" label="录音下发" name="record">
            <div>
              对讲模式：
              <el-select v-model="broadCastModel" placeholder="请选择" :disabled="talkStatus">
                <el-option v-for="item in broadCastOption" :key="item.key" :label="item.key" :value="item.value">
            ...
            <div class="talk-div" @click="recordHandle">
              <i :class="'el-icon-video-play'" :title="recordTitle" v-if="recordStatus == false"></i>

              <i :class="'el-icon-video-pause'" :style="{ color: recordColorStatus ? '' : 'rgba(255, 0, 0, 0.742)' }"
                 :title="recordTitle" v-if="recordStatus == true"></i>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            canTalk() {
                ...
                if (!ishttps || !this.hasAudio) {
                    this.talkTitle = "由于浏览器安全策略本站必须得是https，并且开启通道音频选项才可以对讲";
                    return false;
                }
                this.talkTitle = "开始对讲";
                return true;
        ...
        created() {
            ...
            this.broadCastModel = this.broadCastOption[0].value;
        },
        data() {
            return {
                broadCastOption: [{
                ...
                recordTitle: "开始录音",
                recordStatus: false,
                recordColorStatus: false,
                recordColorStatusInterval: null
            };
        },
        methods: {
            ...
            openDialog: function (tab, deviceId, channelId, param) {
                ...
                if (param.manufacture === 'Dahua') {
                    this.broadCastModel = this.broadCastOption[1].value;
                }
                ...
                this.$axios({
                    ...
                    _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_CONNECTION_STATE_CHANGE, function (state) {// RTC 状态变化 ,详情参考 https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/connectionState
                        ...
                        if (state === "connected") {
                            ...
                            _that.$axios({
                                method: 'get',
                                url: `/api/talkback/start/${_that.deviceId}/${_that.channelId}/${app}/${stream}/${_that.broadCastModel}`
                            }).then(res => {
                                if (res.data.code != 0) {
                                    _that.$message({
                                        message: res.data.msg,
            ...
            stopTalk() {
                ...
                if (this.zlmRTCClient) {
                    ...
                    // this.$axios({
                    //   method: 'get',
                    //   url: `/api/talkback/stop/${this.deviceId}/${this.channelId}/${this.broadCastModel}`
                    // }).then(res => {
            ...
            recordHandle() {
                if (!this.canTalk) {
                    alert(this.talkTitle);
                    return
                }

                this.recordStatus = !this.recordStatus;

                if (this.recordColorStatusInterval != null) {
                    clearInterval(this.recordColorStatusInterval)
                }
                this.recordColorStatusInterval = setInterval(() => {
                    this.recordColorStatus = !this.recordColorStatus;
                }, 500);
            }
